...
let planetMeshes: { mesh: THREE.Mesh; data: PlanetOrbit }[] = [];
...
const initThree = () => {
  ...
  const planetOrbitData: PlanetOrbit[] = [
    {
  ...
  const animate = (time: number) => {
    ...
    stars.forEach((star, idx) => {
      ...
      material.emissiveIntensity = (Math.sin(twinkle + idx * 0.5) + 1) * 0.375;
    });
    if (starsGroup) {
    ...
    planetMeshes.forEach(({ mesh, data }) => {
      ...
      const { rotation } = mesh;
      rotation.y += delta * (1 / Math.max(0.1, speed));
    });
